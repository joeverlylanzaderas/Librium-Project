// frontend/web/src/components/librarian/LibrarianFinesScreen.tsx
...
export default function LibrarianFinesScreen() {
  ...
  // Query
  const { data: finesResponse, isLoading, refetch } = useQuery({
    queryKey: ['fines'],
    queryFn: getFines,
    ...
    <div className="bg-[#FCFAEE] min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        {unpaidFines.length > 0 && (
          <div className="px-4 md:p-6 pt-0 mb-6">
              ...
              {unpaidFines.map((fine, idx) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="bg-[#3D5A45] text-white px-3 py-1.5 text-[11px] font-semibold hover:bg-[#2E4A35] transition-colors disabled:opacity-50 self-start sm:self-center rounded"
                  >
                    Mark Paid
                  </button>
                </div>
        ...
        {fines.length === 0 && (
          <div className="text-center py-12 bg-white border border-[#EFE9CE] mx-4 md:mx-6 rounded-lg">
            <TrendingUp size={48} className="mx-auto text-[#A1927F] mb-3" />
